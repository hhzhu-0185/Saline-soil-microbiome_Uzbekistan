---
title: "Fig.S21_PICRUSt2-supplementary"
author: "Huanhuan Zhu"
date: "2025-02-17"
...
#############################Fig.S20_PICRUSt2-supplementary##########################
...
p1 <- ggplot(data = data,aes(x=mean,y=level2,fill=factor(type, levels = c("MC", "TS")))) +
      ...
      theme(plot.margin = margin(0, 0, 0, 0, "pt"),
            ...
            axis.title.y = element_blank(),  # ?舜???为blank?岬???图片?????锌崭?
...
diff_res<- merge(diff_res,kegg[,c("pathway_id","level2","level1")],by="pathway_id")  # ??????kegg注?秃喜?
diff_res <- diff_res %>%
  filter(level1 != "Human Diseases" & level1 != "Organismal Systems"& level2 != "Cellular community - eukaryotes") %>%
  select(-level1)
...
ko_phylum4_1 <- data.frame(level2 = unique(data$level2)[!(unique(data$level2) %in% unique(ko_phylum4$level2))], 
                           ...
                           value=rep(0,4))  # ??????????????通路??????????通路??目???????诖舜?????没??????????ko??通路
...
p3 <- ggplot(data = ko_phylum4, aes(x = factor(type), y = factor(level2,levels = rev(order)), color = variable, size=value)) +
      ...
      #geom_text(p.data,mapping=aes(x=x+0.012,y=level2,label=as.character(asterisk)),col="#FFA500",vjust=0.7) +  # vjust为?????碌???y??
      #annotate(geom = "text",x=0.12,y=midpoint+1,label="Wildland > Farmland",size=2.5,color="#FFA500") +
      #annotate(geom = "text",x=0.12,y=midpoint-1,label="Farmland < Wildland",size=2.5,color="#FFA500") +
      theme_bw() +
      theme(plot.margin = margin(0, 0, 0, 0, "pt"),
