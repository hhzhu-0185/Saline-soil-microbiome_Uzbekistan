...
#plot
p_amf <- ggplot(df, aes(x=Cor,color=factor(group, levels = c("TS", "MC")))) +
         geom_density(size=1) +
         geom_vline(aes(xintercept=mean,color=factor(group)), linetype= 5, size=1) +
         scale_color_manual(name="Type",values = c("#FFA500","#008000"),
                              labels = c(
                                         "MC"="Farmland (Cotton & Maize)", 
                                         "TS"="Wildland (Tamarix & Suaeda)")) +
         theme_bw() +
         labs(x="Spearman's Rho", y="Density",title = "AM fungi") +
         theme(
...
p_bac <-  ggplot(df, aes(x=Cor,color=factor(group, levels = c("TS", "MC")))) +
          geom_density(size=1) +
          geom_vline(aes(xintercept=mean,color=factor(group)), linetype= 5, size=1) +
          scale_color_manual(name="Type",values = c("#FFA500","#008000"),
                             labels = c(
                               ...
                               "TS"="Wildland (Tamarix & Suaeda)")) +
          theme_bw() +
          labs(x="Spearman's Rho", y="Density",title = "Prokaryotes") +
          theme(
            plot.title = element_text(size = 20,color = "black",face = "bold", hjust = 0.5), 
            axis.title = element_text(size = 15, face = "bold"),
            axis.text = element_text(colour = "black",size = 12,face = "bold"),
...
fungi.network<-function(k, para){
  ...
  #percent transfer
  data0<-data.frame(t((apply(otu_all,2,function(x){x / sum(x)}))),check.names = FALSE)#2 indicates cols; ???ɵ???OTUΪcol????ƷΪrow
...
p_fung <- ggplot(df, aes(x=Cor,color=factor(group, levels = c("TS", "MC")))) +
          geom_density(size=1) +
          geom_vline(aes(xintercept=mean,color=factor(group)), linetype= 5, size=1) +
          scale_color_manual(name="Type",values = c("#FFA500","#008000"),
                             labels = c(
...
p_prot <- ggplot(df, aes(x=Cor,color=factor(group, levels = c("TS", "MC")))) +
          geom_density(size = 1) +
          geom_vline(aes(xintercept = mean,color = factor(group)), linetype = 5, size = 1) +
          scale_color_manual(name="Type",values = c("#FFA500","#008000"),
                             labels = c(
