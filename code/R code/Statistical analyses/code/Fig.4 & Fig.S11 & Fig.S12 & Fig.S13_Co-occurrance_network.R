...
#############################Fig.4 & Fig.S11 & Fig.S12 & Fig.S13_Co-occurrance_network##########################
...
property <- function(title, para){
  ...
  net.metrics$id <- factor(net.metrics$id, levels = c("TS", "MC"))
  ...
  p2 <<- ggplot(net.metrics ) +
    ...
    scale_x_discrete(labels = c("MC" = "Farmland\n(Cotton & Maize)", "TS" = "Wildland\n(Tamarix & Suaeda)" )) +
    scale_y_continuous(
