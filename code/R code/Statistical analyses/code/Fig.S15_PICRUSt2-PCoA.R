...
rletter.data <- data.frame(
  label = c(sprintf(" 'Type:' ~ italic(R)^2 ~ '=' ~ %.3g ~ ~ (italic(F) == %.3g)", 
            permanova["Model","R2"],
            permanova["Model","F"])), 
...
df <- data.frame(p_value = c(permanova["Model","Pr(>F)"]),
                 y = c(range(site[,"pcoa2"])[2] + 0.1 * diff(range(site[,"pcoa2"]))),
...
p2 <- ggplot(data = site, aes(x = pcoa1, y = pcoa2)) +
      geom_point(aes(colour = factor(plants,levels = c("Bare land","Suaeda","Tamarix chinensis","Cotton","Maize")),
                     shape = factor(type,levels = c("Wildland","Farmland"))), size = 7, alpha = 0.8) +
      ...
      scale_shape_manual(name = "Type",
                         values = c(17,16),
                        # label=c("Farmland" = "Farmland (Cotton & Maize)","Wildland" = "Wildland (Tamarix & Suaeda)"),
