...
#############################Fig.S16_PICRUSt2-supplementary##########################
...
p$p.adj <- p.adjust(p$t_test_result.p.value, method = "fdr")  # pֵ????
...
data$level2 <- factor(data$level2,levels = rev(order))
...
p1 <- ggplot(data = data,aes(x=mean,y=level2,fill=factor(type, levels = c("MC", "TS")))) +
      ...
      geom_text(p.data,mapping=aes(x=x+0.012,y=level2,label=as.character(asterisk)),size=5,col="#FFA500",vjust=0.7) +  # vjustΪ?????µ???y??
      annotate(geom = "text",x=0.12,y=midpoint+1,label="Wildland (Tamarix & Suaeda) >\nFarmland (Cotton & Maize)",size=4,color="#FFA500") +
      annotate(geom = "text",x=0.12,y=midpoint-1,label="Farmland (Cotton & Maize) >\nWildland (Tamarix & Suaeda)",size=4,color="#FFA500") +
      theme_bw() + 
      theme(plot.margin = margin(0, 0, 0, 0, "pt"),
            ...
            axis.title.y = element_blank(),  # ?˴???Ϊblank?ᵼ??ͼƬ?????пո?
...
diff_res<- merge(diff_res,kegg[,c("pathway_id","level2","level1")],by="pathway_id")  # ??????keggע?ͺϲ?
diff_res <- diff_res %>%
  filter(level1 != "Human Diseases" & level1 != "Organismal Systems"& level2 != "Cellular community - eukaryotes") %>%
  select(-level1)
...
p2 <- ggplot(data = diff_res,aes(x=log2FoldChange,y=factor(level2,levels = rev(order)),color=factor(change, levels = c("wildland", "farmland")))) +
      geom_point(position = position_jitter(height = 0.2),size=2, alpha=0.5) + 
      #geom_point(size=1, alpha=0.5) +
      scale_color_manual(name="Significance",
                         values = c( "wildland"="#FFA500", "farmland"="#008000"),
                         label= c("wildland"="Wildland (Tamarix & Suaeda)","farmland"="Farmland (Cotton & Maize)")) +
      geom_hline(yintercept = midpoint, linetype = "dashed", color = "#FFA500") +
...
p2_1 <- ggplot(data = diff_res,aes(x=log2FoldChange,y=factor(level2,levels = rev(order)),color=factor(change, levels = c("wildland", "farmland")))) +
        geom_point(position = position_jitter(height = 0.2),size=2, alpha=0.5) + 
        #geom_point(size=1, alpha=0.5) +
        scale_color_manual(name="Significance",
                           values = c( "wildland"="#FFA500", "farmland"="#008000"),
...
ko_phylum4_1 <- data.frame(level2 = unique(data$level2)[!(unique(data$level2) %in% unique(ko_phylum4$level2))], 
                           ...
                           value=rep(0,4))  # ??????????????ͨ·??????????ͨ·??Ŀ???????ڴ˴?????û??????????ko??ͨ·
ko_phylum4 <- rbind(ko_phylum4, ko_phylum4_1)
ko_phylum4$level2 <- factor(ko_phylum4$level2, levels = levels(data$level2))

set.seed(23) 
p3 <- ggplot(data = ko_phylum4, aes(x = factor(type), y = factor(level2,levels = rev(order)), color = variable, size=value)) +
      ...
      #geom_text(p.data,mapping=aes(x=x+0.012,y=level2,label=as.character(asterisk)),col="#FFA500",vjust=0.7) +  # vjustΪ?????µ???y??
      #annotate(geom = "text",x=0.12,y=midpoint+1,label="Wildland > Farmland",size=2.5,color="#FFA500") +
      #annotate(geom = "text",x=0.12,y=midpoint-1,label="Farmland < Wildland",size=2.5,color="#FFA500") +
      theme_bw() +
      theme(plot.margin = margin(0, 0, 0, 0, "pt"),
...
p3_1 <- ggplot(data = ko_phylum4, aes(x = factor(type), y = factor(level2,levels = rev(order)), color = variable, size=value)) +
        scale_size(range = c(0, 3))+
        scale_color_manual(values = c(as.vector(polychrome(12))[3:12],"grey")) +
        geom_point(position = position_jitter(width = 0.4, height = 0.3)) +
        guides(size = "none",
